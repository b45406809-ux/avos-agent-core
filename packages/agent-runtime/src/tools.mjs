...
// ============================================================================
// 1. Tool Schemas (Claude Code / OpenCode Standard)
// ============================================================================
export const CLAUDE_TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: "bash_exec",
      description: "Execute a shell command inside the workspace directory (e.g., 'npx wrangler deploy', 'pnpm test', 'cargo test'). Custom secrets are auto-injected.",
      parameters: {
        type: "object",
        properties: {
          command: {
  ...
  },
  {
    ...
    function: {
      name: "finish_goal",
      description: "Signal that the assigned unit of work is completed and verified.",
      parameters: {
        type: "object",
        properties: {
          summary: {
...
// Security list of disallowed commands
...
export async function executeTool(name, args = {}, workspaceDir = "./workspace") {
  ...
  try {
    // ------------------------------------------------------------------------
    // glob_files (Supports git-ls and directory matching)
    // ------------------------------------------------------------------------
    if (name === "glob_files") {
      const pattern = args.pattern || "*";

      // Try git ls-files first
      const gitRes = spawnSync("git", ["ls-files", pattern, `*${pattern}*`], {
        cwd: resolvedWorkspace,
        encoding: "utf-8",
        timeout: 10000
      });

      let files = (gitRes.stdout || "").trim().split("\n").filter(Boolean);
      ...
      if (files.length === 0) {
        ...
        if (pattern.includes("/")) {
          findArgs.push("-ipath", `*${pattern}*`);
        } else {
          findArgs.push("-iname", `*${pattern}*`);
        }

        const findRes = spawnSync("find", findArgs, {
          cwd: resolvedWorkspace,
          encoding: "utf-8",
          timeout: 15000
        });

        files = (findRes.stdout || "")
          .trim()
          .split("\n")
          .filter(Boolean)
          .map(f => f.replace(/^\.\//, ""));
      ...
      const clean = Array.from(new Set(files)).slice(0, 50);
      return clean.length > 0 ? clean.join("\n") : `No files found matching '${pattern}'.`;
    }

    // ------------------------------------------------------------------------
    // grep_search (Ripgrep with workspace scoping)
    // ------------------------------------------------------------------------
    if (name === "grep_search") {
      const searchTarget = args.path ? args.path : ".";
      ...
      const res = spawnSync("rg", [
        ...
        "--glob", "!dist",
        "--glob", "!.worktrees",
        "--glob", "!.env*", // Prevent leaking local env files in search
        "-i",
      ...
      return (res.stdout || res.stderr || "").trim() || `No matches found for '${args.query}'.`;
    }

    // ------------------------------------------------------------------------
    // get_project_map (Tree-Sitter AST Skeleton)
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // get_file_outline (Tree-Sitter Signatures)
    // ------------------------------------------------------------------------
    ...
    if (name === "read_file_slice") {
      ...
      // Block AI from reading sensitive injected secret files directly
      if (path.basename(fullPath).startsWith(".env.secret")) {
        return "Error: Access to credential files is restricted.";
      }
      ...
      if (stat.size > 2 * 1024 * 1024) {
        return `Error: File '${args.filePath}' exceeds 2MB limit. Use 'get_file_outline' or 'grep_search'.`;
      }
      ...
      return `[File: ${args.filePath} (Lines ${offset} to ${Math.min(offset + limit - 1, lines.length)} of ${lines.length})]\n${formatted}`;
    }

    // ------------------------------------------------------------------------
    // edit_file (Surgical string replace with context verification)
    // ------------------------------------------------------------------------
    if (name === "edit_file") {
      ...
      if (firstIndex === -1) {
        return `Error: 'oldStr' not found in ${filePath}.\nMake sure exact indentation and whitespace match. Use 'read_file_slice' first.`;
      }
    ...
    // ------------------------------------------------------------------------
    // bash_exec (Auto-injects custom secrets into execution subshell)
    // ------------------------------------------------------------------------
    if (name === "bash_exec") {
      ...
      // Parse custom environment variables from process.env.CUSTOM_ENV_JSON
      let customEnvs = {};
      try {
        if (process.env.CUSTOM_ENV_JSON) {
          customEnvs = JSON.parse(process.env.CUSTOM_ENV_JSON);
        }
      } catch (_) {}

      // Execute inside resolved workspace with custom secrets exported in subshell
      const res = spawnSync("bash", ["-c", command], {
        ...
        env: {
          ...process.env,
          ...customEnvs, // <-- Injects custom Cloudflare / DB secrets
          CI: "true",
          TERM: "xterm-256color"
        }
      });
